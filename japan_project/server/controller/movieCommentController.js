...
const getMovieCommentsById = async (req, res) => {
  ...
  try {
    const movieById = await MovieCommentModel.find({ movie_id: id })
      .populate({ path: "user_id", select: ["name", "imageUrl"] })
      .exec();
...
const postMovieComment = async (req, res) => {
  const { language_level, movie_id, rating, user_id, user_imageUrl, comment } =
    req.body;
...
const deleteMovieComment = async (req, res) => {
  ...
  const commentObjectId = new mongoose.Types.ObjectId(id); //REVIEW is it really necessary to turn the id from the request into an objectID? i think mongoose understand it anyway
  // console.log("Converted userObjectId:", commentObjectId);

  try {
    const newDelete = await MovieCommentModel.deleteOne({
      _id: commentObjectId,
    });
    if (newDelete) {
      return res.status(201).json({
...
export { getMovieCommentsById }; //REVIEW, same comment as before, you can export them all in the same line
